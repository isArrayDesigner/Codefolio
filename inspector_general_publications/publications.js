...
let requestURI1 =
    "/InspectorGeneral/_api/web/lists/getbytitle('Publications')/items?$top=300&$orderBy=Title",
  requestURI2 =
    "/InspectorGeneral/_api/web/getfolderbyserverrelativeurl('Publications')/files?&$orderBy=Title"
...
$.when(req1, req2).done(function (resp1, resp2) {
  ...
  selectResults =
    selectResults.length <= 1 ? selectResults['0'] : selectResults[1]
  ...
  function yearNav () {
    ...
    $(
      '<select style="border:none; height:50px; font-family:Arquette;" class="browser-default w-100 mdb-select yearSelect" />'
    ).appendTo('#yearNav')
...
let buildResults = function () {
  ...
  tHeaderRow.append(
    $(
      '<th>Title</th><th>Publication Type</th><th>Year</th><th>Issued</th><th>PDF</th>'
    )
  )
    ...
    $.each(searchResults[0], function (index, resultItems) {
      $('#rowItems').append(
        $('<tr>')
          ...
          .append(
            $('<td class="align-middle">' + resultItems.category + '</td>')
          )
          ...
          .append(
            $(
              '<td class="align-middle"><a href="' +
                resultItems.document +
                '"><i class="fal fa-file-pdf fa-2x broward-color-blue"></i></a></td>'
            )
          )
      )
    })
